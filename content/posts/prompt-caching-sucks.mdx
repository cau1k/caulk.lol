...
<Accordions>
  <Accordion title="CacheBlend (Liu et al.)">
    Precompute KV tensors per chunk (per file, per tool output, per document). At runtime, concatenate chunk KVs—even if they weren't originally a prefix—and selectively recompute a small subset of tokens to recover cross-chunk attention. 2-3x faster TTFT, 3-5x throughput, negligible quality loss. [Paper](https://arxiv.org/abs/2405.16444)
  </Accordion>
</Accordions>

<Accordions>
  <Accordion title="EPIC (Jin et al.)">
    Formalizes "position-independent caching" and introduces LegoLink: recompute only the first ~32 tokens at each chunk boundary to fix attention sink artifacts. Up to 8x TTFT improvement. [Paper](https://arxiv.org/abs/2410.15332)
  </Accordion>
</Accordions>

<Accordions>
  <Accordion title="Block-Attention (Lee et al.)">
    Changes the attention mask so retrieved passages are independent blocks with no cross-block attention. Only the final block (your query) attends to everything. Requires fine-tuning, but cached passage KVs become reusable across any context. 98.7% TTFT reduction. [Paper](https://arxiv.org/abs/2409.15355)
  </Accordion>
</Accordions>

<Accordions>
  <Accordion title="KVLink (Chen et al.)">
    Store KV without baked-in positional encoding (RoPE), then reapply correct positions at inference and insert trainable "link tokens" between segments. 85-96% TTFT reduction with precomputed caches. [Paper](https://arxiv.org/abs/2502.16002)
  </Accordion>
</Accordions>
