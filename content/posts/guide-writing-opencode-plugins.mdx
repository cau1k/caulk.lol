...
OpenCode plugins are Bun/TypeScript modules that run inside the harness process. This guide shows every feature you can use: manifests, hooks, tools, auth, permissions, events, cancellation, testing, and packaging—plus MDX components you can copy into your own docs.
...
<Callout type="warning" title="Reality check">
  Plugins run in the same Bun process as OpenCode. No sandbox. Validate inputs, respect AbortSignals, and avoid unsafe shell calls.
</Callout>

## Fast path: scaffold in 6 steps
...
<Tabs items={["Overview", "Full Example", "Unknowns"]} label="Entrypoint">
  ...
  <Tab>
    Unknowns: no direct UI/MDX injection from plugins; streaming tool outputs are not exposed; telemetry hooks are not published.
  </Tab>
</Tabs>
...
<TypeTable
  type={{
    ...
    directory: {
      description: "Current working directory for this invocation.",
...
- Use template literals; each substitution is shell-escaped.
...
<Accordions type="multiple">
  ...
  <Accordion title="permission.ask" id="hook-permission">
    Intercept permission requests and allow/deny/ask.
  </Accordion>
...
- Permission types include `bash`, `edit`, `webfetch`, `doom_loop`, `external_directory`.
- Use to auto-allow safe patterns and deny risky ones.
...
- No built-in retries or streaming for tools—implement manually if needed.
...
<Callout type="error" title="Known gaps">
  ...
  - Telemetry hooks are not documented; rely on your own logging.
</Callout>
...
- Keep handlers pure; avoid mutable singletons.
