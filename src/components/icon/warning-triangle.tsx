import * as React from "react";
export const AlertCircle = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 32 32"
    id="Interface-Essential-Alert-Triangle-2--Streamline-Pixel"
    height={32}
    width={32}
    {...props}
  >
    <desc>
      {
        "\n    Interface Essential Alert Triangle 2 Streamline Icon: https://streamlinehq.com\n  "
      }
    </desc>
    <title>{"interface-essential-alert-triangle-2"}</title>
    <g>
      <path
        d="M30.48 24.38h-1.53v-3.05h-1.52v-3.04H25.9v-3.05h-1.52v-3.05h-1.52V9.14h-1.53V6.09h-1.52V3.05h-1.52V1.52h-1.53V0h-1.52v1.52h-1.53v1.53h-1.52v3.04h-1.52v3.05H9.14v3.05H7.62v3.05H6.09v3.05H4.57v3.04H3.05v3.05H1.52v3.05H0v3.05h1.52V32h28.96v-1.52H32v-3.05h-1.52Zm-10.67 3.05h-1.52v1.52h-4.58v-1.52h-1.52v-4.57h1.52v-1.53h4.58v1.53h1.52Zm0 -10.67h-1.52v3.05h-4.58v-3.05h-1.52v-6.09h1.52V9.14h4.58v1.53h1.52Z"
        fill={currentColor}
        strokeWidth={1}
      />
      <path
        d="M16.76 12.19h1.53v3.05h-1.53Z"
        fill={currentColor}
        strokeWidth={1}
      />
      <path
        d="M15.24 10.67h1.52v1.52h-1.52Z"
        fill={currentColor}
        strokeWidth={1}
      />
    </g>
  </svg>
);
