...
I'm usually $O(n^2)$ deep in tasks, so compute gets tied up.

So I started uploading to my servers, but doing this manually means a lot of `rsync`, `ssh`, `tmux attach`, copy-pasting commands. 
...
```bash
# upload and run
./bench-runner oracle -p tasks/my-benchmark --no-debug

# stream logs
./bench-runner oracle -p tasks/my-benchmark --stream

# kill session
./bench-runner oracle -p tasks/my-benchmark --kill
```
...
The whole thing is ~200 lines and took maybe 15 minutes including debugging tmux output issues. Most of that time was figuring out runner permissions issues on my Azure VM - nothing to do with Bun.
